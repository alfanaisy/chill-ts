...
const VListItem = ({ image, isTop = false, labels, title, width }: Props) => {
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-full h-3/4 md:h-1/2 p-4 bg-black bg-opacity-80 text-white rounded-md opacity-0 transform translate-y-4 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">
        <h3 className="text-sm md:text-base font-bold">{title}</h3>
        <div className="mt-2 flex space-x-2 items-center justify-around">
          <button className="h-5 w-5 md:h-8 md:w-8 bg-white rounded-full flex-none flex items-center justify-center text-2xl text-black">
            <BiPlay />
          </button>
          <button className="px-2 py-1 md:px-3 md:py-1.5 text-sm md:text-base bg-gray-700 text-white rounded">
            Details
          </button>
        </div>
      </div>
